 ...
 */
public class TemaHttp {
    public static final String LIVROS_URL_JSON =
            "http://www.ase-jf.com.br/gpp/tema.php";
    ...
    public static List<TemaDTO> carregarLivrosJson() {
        try {
    ...
    public static List<TemaDTO> lerJsonLivros(JSONObject json) throws JSONException {
        List<TemaDTO> listaDeLivros = new ArrayList<TemaDTO>();
      ...
      //  for (int i = 0; i < jsonNovatec.length(); i++) {
        //    JSONObject jsonCategoria = jsonNovatec.getJSONObject(i);
           // categoriaAtual = jsonCategoria.getString("categoria");
            JSONArray jsonLivros = json.getJSONArray("temas");
            for (int j = 0; j < jsonLivros.length(); j++) {
                JSONObject jsonLivro = jsonLivros.getJSONObject(j);
                TemaDTO TemaDTO = new TemaDTO();
                TemaDTO.setIdTema(jsonLivro.getInt("idTema"));
                TemaDTO.setTema(jsonLivro.getString("nomeTema"));
                listaDeLivros.add(TemaDTO);
            }
   //     }
